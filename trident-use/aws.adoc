...
| `apiRegion` | CVS 帐户区域。您可以在 CVS Web 门户中的帐户设置 /API 访问中找到此值。 |  
...
| `secretKey` | CVS 帐户密钥。您可以在 CVS Web 门户中的帐户设置 /API 访问中找到此值。 |  
...
NOTE:  `apiURL` 每个都是唯一的 `apiRegion`。例如、us-west-2 `apiRegion` 具有 https://cv.us-west-2.netapp.com:8080/v1/[] `apiURL`。同样、也是us-east-1 `apiRegion` 具有 https://cds-aws-bundles.netapp.com:8080/v1/[] `apiURL`。确保检查CVS信息板是否正确 `apiRegion` 和 `apiURL` 用于后端配置的参数。
...
| `snapshotDir` | 控制的可访问性 `.snapshot` 目录 | false 
...
此示例显示了一个后端文件，该文件对 AWS us-east-1 区域中由 Astra Trident 创建的所有存储应用相同的方面。此示例还显示了的使用情况 `proxyURL` 在后端文件中。
