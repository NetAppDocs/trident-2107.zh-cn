...
从 Astra Trident 的 20.01 版开始，您可以在 Kubernetes 层创建 PV 快照。您可以使用这些快照维护由 Astra Trident 创建的卷的时间点副本，并计划创建其他卷（克隆）。支持卷快照 `ontap-nas`， `ontap-san`， `ontap-san-economy`， `solidfire-san`， `aws-cvs`， `gcp-cvs`，和 `azure-netapp-files` 驱动程序。
