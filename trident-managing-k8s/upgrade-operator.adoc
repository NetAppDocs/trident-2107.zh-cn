...
* 您应使用运行的 Kubernetes 集群  link:../trident-get-started/requirements.html["1.17 及更高版本"^]。
...
NOTE: 状态显示时 `Updating`、请确保先解决此问题、然后再继续。有关可能的状态值列表，请参见 link:../trident-get-started/kubernetes-deploy-operator.html["此处"^]。
...
NOTE: 您应对Trident安装参数进行必要的更改(例如、更改的值 `tridentImage`， `autosupportImage`、私有映像存储库和提供 `imagePullSecrets`)。有关可更新的参数的完整列表，请参见 link:../trident-get-started/kubernetes-customize-deploy.html["参数列表"^]。
