...
IMPORTANT: 如果您使用的是私有映像存储库、则应添加 `/k8scsi` 适用于1.17或之前的Kubernetes版本 `/sig-storage` 对于1.17之后的Kubernetes版本、请转到专用注册表URL末尾。使用私有注册表时 `tridentctl` 部署、您应使用 `--trident-image` 和 `--autosupport-image` 与结合使用 `--image-registry`。如果要使用Trident操作符部署Astra Trident、请确保Orchestrator CR包括在内 `tridentImage` 和 `autosupportImage` 在安装参数中。
