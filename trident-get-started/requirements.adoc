...
NOTE: 在Kubernetes 1.20及更高版本上、使用经验证的 `k8s.gcr.io/sig-storage/csi-snapshotter:v4.x` 仅当出现时才创建映像 `v1` 版本正在提供 `volumesnapshots.snapshot.storage.k8s.io` CRD。如果 `v1beta1` 版本正在为CRD提供支持/不提供 `v1` 版本、请使用已验证的 `k8s.gcr.io/sig-storage/csi-snapshotter:v3.x` 图像。
